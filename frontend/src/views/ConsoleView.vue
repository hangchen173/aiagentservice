...
function connect(){if(!selectedSession.value)return;const version=++socketVersion;socket?.close();const scheme=location.protocol==='https:'?'wss':'ws';const active=new WebSocket(`${scheme}://${location.host}/ws/chat?token=${encodeURIComponent(auth.token)}`);socket=active;active.onopen=()=>active.send(JSON.stringify({type:'JOIN_SESSION',sessionId:selectedSession.value?.id}));active.onmessage=async event=>{const payload=JSON.parse(event.data);if(['CHAT_MESSAGE','SYSTEM_NOTICE','TICKET_CREATED','AGENT_ACCEPTED','SESSION_CLOSED'].includes(payload.type)){await reloadSelected();await refreshCore()}};active.onclose=()=>{if(version===socketVersion&&selectedSession.value)window.setTimeout(connect,1500)}}
...
<style scoped>
.console-workspace{grid-template-columns:220px minmax(0,1fr)}.console-sidebar{padding:18px 10px}.console-sidebar .el-menu{border-right:0;background:transparent}.console-main{padding:28px}.page-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin-bottom:22px}.page-heading h1{margin:0;font-size:28px}.page-heading p{margin:7px 0 0;color:#86868b}.heading-actions,.account-actions,.ticket-actions{display:flex;align-items:center;gap:10px}.online-text{font-size:18px!important;color:#16803c}.support-desk{display:grid;grid-template-columns:320px minmax(0,1fr);height:620px;border:1px solid #dedee3;border-radius:8px;background:#fff;overflow:hidden}.queue-pane{padding:16px;border-right:1px solid #e5e5e7;overflow:auto}.pane-title{display:flex;justify-content:space-between;margin-bottom:14px}.queue-pane .el-select{width:100%;margin-bottom:12px}.queue-item{display:flex;width:100%;align-items:center;justify-content:space-between;gap:8px;padding:12px 10px;border:0;border-radius:7px;background:transparent;text-align:left;cursor:pointer}.queue-item:hover,.queue-item.active{background:#f0f0f3}.queue-item span,.queue-item strong,.queue-item small{display:block;min-width:0}.queue-item strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.queue-item small{margin-top:4px;color:#86868b}.agent-chat-pane{display:grid;grid-template-rows:auto minmax(0,1fr) auto;min-width:0}.agent-chat-header{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:18px 22px;border-bottom:1px solid #e5e5e7}.agent-chat-header h2{margin:0;font-size:18px}.agent-chat-header p{margin:5px 0 0;color:#86868b;font-size:13px}.agent-messages{padding:20px}.agent-composer{align-items:flex-end;padding:14px 18px 18px}.pane-empty{padding:28px;text-align:center;color:#86868b}.pane-empty.large{display:grid;min-height:100%;place-items:center}.data-panel{padding:18px;border:1px solid #dedee3;border-radius:8px;background:#fff}.table-toolbar{display:flex;justify-content:flex-end;margin-bottom:14px}.table-toolbar .el-select{width:180px}.route-preview{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;margin-bottom:14px}.preview-result{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:16px}.preview-result div{padding:12px;border:1px solid #e5e5e7;border-radius:7px}.preview-result span,.preview-result strong{display:block}.preview-result span{color:#86868b;font-size:12px}.preview-result strong{margin-top:5px}.message-image{display:block;width:min(320px,100%);max-height:280px;margin:8px 0;border-radius:7px;object-fit:contain}@media(max-width:900px){.console-workspace{grid-template-columns:1fr}.console-sidebar{padding:6px 12px;border-bottom:1px solid #dedee3}.console-sidebar .el-menu{display:flex;overflow-x:auto}.console-main{padding:16px}.support-desk{grid-template-columns:260px minmax(420px,1fr);overflow-x:auto}}@media(max-width:650px){.page-heading{align-items:stretch;flex-direction:column}.heading-actions{justify-content:flex-end}.support-desk{display:block;height:auto}.queue-pane{max-height:300px;border-right:0;border-bottom:1px solid #e5e5e7}.agent-chat-pane{height:600px}.preview-result{grid-template-columns:repeat(2,minmax(0,1fr))}}
</style>
